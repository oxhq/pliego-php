...
and host-font discovery. Fetch remote resources before the render and supply their bytes through
`assets`. `allowedHttpRoots` remains only for the deprecated API 1 compatibility client and is
rejected by `DocumentEngine` with a migration message.
